...
const coding = new (require('./codingFunctions'));


function PEMToHex(key, kind = 'PUBLIC') {
    let k = coding.PEM.decode(key, `rsa ${kind} key`);
    let hex = coding.Hex.encode(k);
    hex = hex.replace(new RegExp(/\r\n/, 'g'), "");
    return hex;
}

function hexToPem(key, kind = 'PUBLIC') {
    key = key.replace(new RegExp(/\r\n/, 'g'), "");
    let k = coding.Hex.decode(key);
...
function validate(data, sign, publicKey) {
    ...
    publicKey = publicKey.indexOf('RSA PUBLIC KEY') < 0 ? hexToPem(publicKey, 'PUBLIC') : publicKey;
...
module.exports = {
    hash: sha256, sign: sign, verify: validate, generate: generateKeyPair, register: () => {
    }
};